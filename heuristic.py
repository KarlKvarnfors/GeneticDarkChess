...
def get_progress(game_state):
    ...
    for row in range(game_state.board_size):
        for col in range(game_state.board_size):
            if (game_state.board[row][col][0] == cell_occupation_code_black ||
                game_state.board[row][col][0] == cell_occupation_code_white ):
                number_of_pieces += 1
    return number_of_pieces/64

def h(game_state, weigths):
    ...
    P = lambda _x : numpy.array([ [bernstein(_k,n)(_x)] for _k in range(n+1) ])
    H = lambda _game_state : numpy.array([
        _heuristic(_game_state) for _heuristic in HEURISTICS
    ])
    x = get_progress(game_state)
    # H(game_state).transpose * W * P(x) :
    return numpy.dot( H(game_state) , numpy.dot(weights,P(x)) )
